...
function createProvisionalZooExcursion(events: FestivalEvent[]) {
  ...
  return {
    ...
    publicationStatus: 'Перенос',
    hiddenFromPublic: false,
    accessLabel: 'Результаты розыгрыша сохраняются',
    dateLabel: 'Дата будет объявлена дополнительно',
    monthLabel: 'Июнь',
    monthAnchor: 'june',
    timeLabel: 'перенос',
    ...
    affiliation: '',
    heroRole: '',
    summary: 'Экскурсия переносится из-за прогнозируемой жары до +37 °C; результаты розыгрыша сохраняются, новая дата будет в выходной день. Премьера лучше раскроет, что появилось в зоопарке в советское время, познакомит с историей зоопарка того периода и покажет вживую, как менялся зоопарк после немецкой эпохи.',
    ...
    icsUrl: undefined,
    calendarNote: 'Экскурсия 28 июня переносится из-за прогнозируемой жары; результаты розыгрыша сохраняются.',
    kind: 'special' as const,
    isoStart: '2026-06-28T14:00:00',
    showingsLabel: 'Перенос · новая дата будет в выходной день',
    speakerImages: [],
